...
def generate_launch_description():
    ...
    time_window_arg = DeclareLaunchArgument('time_window_sec', default_value='10.0')
    time_window_sec = LaunchConfiguration('time_window_sec')

    max_age_arg = DeclareLaunchArgument('max_age_sec', default_value='30.0')
    ...
    for ns in drone_list:
        groups.append(
            GroupAction([
                PushRosNamespace(ns),
                Node(
                    package='as2_swarm_ekf',
                    executable='distributed_ekf_node',
                    name='distributed_ekf',
                    output='screen',
                    parameters=[{
                        ...
                        'world_frame': 'earth',
                        'publish_topic': f'/swarm/{ns}/ekf_target',
